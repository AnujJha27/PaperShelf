...
type RunWarning = { provider?: string; category?: string; feed_name?: string; message?: string };
type DiagnosticStats = { feeds_attempted?: number; feeds_succeeded?: number; feeds_degraded?: number; candidates_in_scope?: number; recommendations_created?: number; warnings?: RunWarning[] };
...
export function SettingsPage() {
  ...
  const [diagnostics, setDiagnostics] = useState<{ runs: Array<{ mode: string; status: string; finished_at: string | null; stats?: DiagnosticStats }>; lastZoteroSync: { status: string } | null; model: { trained_at?: string } | null; apiErrors: Array<{ mode: string; error?: string }>; guardrails: { max_feed_recommendations?: number; max_today_recommendations?: number; exploration_rate?: number } | null }>();
