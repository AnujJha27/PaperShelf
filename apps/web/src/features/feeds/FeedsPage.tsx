...
export function FeedsPage() {
  ...
  return <PageContainer>
    ...
    <Paper component="form" onSubmit={submit} sx={{ p: { xs: 2, sm: 3 }, mb: 4 }}><Stack spacing={2}>
      ...
      <Stack direction={{ xs: "column", sm: "row" }} spacing={2} sx={{ alignItems: { xs: "stretch", sm: "center" } }}><FormControl sx={{ minWidth: 250 }}><InputLabel id="similarity-label">Minimum similarity</InputLabel><Select labelId="similarity-label" label="Minimum similarity" value={String(form.min_semantic_similarity ?? 0.35)} onChange={(event) => setForm({ ...form, min_semantic_similarity: Number(event.target.value) })}>{[0.25, 0.35, 0.45, 0.55, 0.65].map((value) => <MenuItem key={value} value={value}>{value.toFixed(2)}</MenuItem>)}</Select></FormControl><Stack direction="row" spacing={1}><Button type="submit" variant="contained">{editing ? "Save feed" : "Create feed"}</Button>{editing && <Button type="button" onClick={() => { setEditing(undefined); setForm(empty); }}>Cancel</Button>}</Stack></Stack>
    </Stack></Paper>
